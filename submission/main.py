...
# V3 research layer: retain v19's current-meta field/market expert and add the
# public-state logistics timing model that was validated on the c27 route.
import copy as _v3_copy

_V3_PARENT_AGENT = agent
_V3_PREV_OPPONENT = None
_V3_CARGO = []
_V3_LAST_STEP = -1
_V3_AUTO_SHED = {}
_V3_AUTO_SHED_STEP = -1
_V3_HORIZON = 8
_V3_ITEMS = ("MELON", "STRAWBERRY", "MILK", "WOOL")
_V3_PRODUCT = {"COW": "MILK", "SHEEP": "WOOL", "GOOSE": "EGG"}


def _v3_positions(farm):
...
def _v3_item_and_yield(tile):
    ...
    return _V3_PRODUCT.get(tile.get("animal")), quantity


def _v3_observe_cargo(opponent, step):
    global _V3_PREV_OPPONENT, _V3_CARGO, _V3_LAST_STEP
    global _V3_AUTO_SHED, _V3_AUTO_SHED_STEP
    current_positions = _v3_positions(opponent)
    if _V3_PREV_OPPONENT is None or step != _V3_LAST_STEP + 1 or step % 24 == 0:
        if (
            _V3_PREV_OPPONENT is not None
            and step == _V3_LAST_STEP + 1
            and step % 24 == 0
        ):
            _V3_AUTO_SHED = {}
            for cargo in _V3_CARGO:
                for item, quantity in cargo.items():
                    _V3_AUTO_SHED[item] = (
                        int(_V3_AUTO_SHED.get(item, 0) or 0)
                        + int(quantity or 0)
                    )
            _V3_AUTO_SHED_STEP = step
        else:
            _V3_AUTO_SHED = {}
            _V3_AUTO_SHED_STEP = -1
        _V3_CARGO = [{} for _ in current_positions]
        _V3_PREV_OPPONENT = _v3_copy.deepcopy(opponent)
        _V3_LAST_STEP = step
        return

    previous_positions = _v3_positions(_V3_PREV_OPPONENT)
    sheds = _v3_sheds(len(opponent.get("tiles") or []))
    if len(_V3_CARGO) < len(current_positions):
        _V3_CARGO.extend({} for _ in range(len(current_positions) - len(_V3_CARGO)))
    for index in range(min(len(previous_positions), len(current_positions))):
        ...
        if previous_position in sheds and _V3_CARGO[index]:
            _V3_CARGO[index] = {}
        item, previous_yield = _v3_item_and_yield(
            _v3_tile(_V3_PREV_OPPONENT, previous_position)
        )
        current_item, current_yield = _v3_item_and_yield(
            _v3_tile(opponent, current_position)
        )
        if (
            item in _V3_ITEMS
            ...
            quantity = (
                ...
                if current_item == item else previous_yield
            )
            _V3_CARGO[index][item] = (
                int(_V3_CARGO[index].get(item, 0) or 0) + quantity
            )
    _V3_CARGO = _V3_CARGO[:len(current_positions)]
    _V3_PREV_OPPONENT = _v3_copy.deepcopy(opponent)
    _V3_LAST_STEP = step


def _v3_declared_sale(action, item):
    return any(
...
def _v3_planned_quantity(obs, item, start, stop):
    seat = 1 if int(obs.get("player", 0) or 0) == 1 else 0
    market_name = _V19_SELECTED_MARKET.get(seat)
    if market_name is None:
        return 0
    actions = _V19_RUNTIME["experts"][market_name]["actions"]
    total = 0
    for future in range(start, min(stop, len(actions))):
...
def _v3_front_run(action, obs, opponent, step):
    orders = list(action.get("market", []) or [])
    if len(orders) >= 10:
        return action
    sheds = _v3_sheds(len(opponent.get("tiles") or []))
    risks = []
    if _V3_AUTO_SHED_STEP == step:
        for item, quantity in _V3_AUTO_SHED.items():
            if item in _V3_ITEMS and quantity > 0:
                risks.append((float(quantity), item, _V3_HORIZON))
    positions = _v3_positions(opponent)
    for index, cargo in enumerate(_V3_CARGO[:len(positions)]):
        if not cargo:
            continue
        x, y = map(int, positions[index])
        distance = min(abs(x - sx) + abs(y - sy) for sx, sy in sheds)
        if not 1 <= distance <= _V3_HORIZON:
            continue
        for item, quantity in cargo.items():
            if item in _V3_ITEMS and quantity > 0:
                risks.append((quantity / distance, item, distance))
    if not risks:
        return action
    shed = (obs.get("private") or {}).get("shed") or {}
    choices = []
    for risk, item, distance in risks:
        if _v3_declared_sale(action, item):
            continue
        planned = _v3_planned_quantity(obs, item, step + 1, step + distance + 1)
        quantity = min(max(0, int(shed.get(item, 0) or 0)), planned)
        if quantity > 0:
            choices.append((risk * quantity, item, quantity))
    if choices:
...
def agent(obs):
    """V19 current-meta route plus observation-only logistics front-running."""
    global _V3_PREV_OPPONENT, _V3_CARGO, _V3_LAST_STEP
    global _V3_AUTO_SHED, _V3_AUTO_SHED_STEP
    step = max(0, int(obs.get("step", 0) or 0))
    if step == 0 or step <= _V3_LAST_STEP:
        _V3_PREV_OPPONENT = None
        _V3_CARGO = []
        _V3_LAST_STEP = -1
        _V3_AUTO_SHED = {}
    ...
    opponent = farms[1 - player]
    _v3_observe_cargo(opponent, step)
    action = _V3_PARENT_AGENT(obs)
    return _v3_front_run(action, obs, opponent, step)
...
def submission_agent(obs):
    """Last newly named callable: Kaggle's raw-file loader selects this one."""
    return agent(obs)
