...
# Opponent-logistics model. Public tiles and worker positions let us confirm
# premium harvests, carry estimated cargo by worker, and anticipate shed sales.
_LOG_PREV_OPPONENT = None
_LOG_CARGO = []
_LOG_LAST_STEP = -1
_LOG_AUTO_SHED = {}
_LOG_AUTO_SHED_STEP = -1
_LOG_PRODUCT = {"COW": "MILK", "SHEEP": "WOOL", "GOOSE": "EGG"}
_LOG_HORIZON = 8
...
def _log_positions(farm):
    return [farm.get("farmer", [0, 0]), *(farm.get("hands", []) or [])]


def _log_shed_tiles(size):
    half = size // 2
    return {(half - 1, half - 1), (half, half - 1), (half - 1, half), (half, half)}


def _log_tile(farm, position):
...
def _log_item_and_yield(tile):
    ...
    return _LOG_PRODUCT.get(tile.get("animal")), quantity


def _observe_opponent_cargo(opponent, step):
    global _LOG_PREV_OPPONENT, _LOG_CARGO, _LOG_LAST_STEP
    global _LOG_AUTO_SHED, _LOG_AUTO_SHED_STEP
    current_positions = _log_positions(opponent)
    if _LOG_PREV_OPPONENT is None or step != _LOG_LAST_STEP + 1 or step % 24 == 0:
        if (
            _LOG_PREV_OPPONENT is not None
            and step == _LOG_LAST_STEP + 1
            and step % 24 == 0
        ):
            _LOG_AUTO_SHED = {}
            for cargo in _LOG_CARGO:
                for item, quantity in cargo.items():
                    _LOG_AUTO_SHED[item] = (
                        int(_LOG_AUTO_SHED.get(item, 0) or 0) + int(quantity or 0)
                    )
            _LOG_AUTO_SHED_STEP = step
        else:
            _LOG_AUTO_SHED = {}
            _LOG_AUTO_SHED_STEP = -1
        _LOG_CARGO = [{} for _ in current_positions]
        _LOG_PREV_OPPONENT = copy.deepcopy(opponent)
        _LOG_LAST_STEP = step
        return

    previous_positions = _log_positions(_LOG_PREV_OPPONENT)
    sheds = _log_shed_tiles(len(opponent.get("tiles") or []))
    if len(_LOG_CARGO) < len(current_positions):
        _LOG_CARGO.extend({} for _ in range(len(current_positions) - len(_LOG_CARGO)))
    for index in range(min(len(previous_positions), len(current_positions))):
        ...
        if previous_position in sheds and _LOG_CARGO[index]:
            _LOG_CARGO[index] = {}
        item, previous_yield = _log_item_and_yield(
            _log_tile(_LOG_PREV_OPPONENT, previous_position)
        )
        current_item, current_yield = _log_item_and_yield(
            _log_tile(opponent, current_position)
        )
        if (
            item in _FRONT_RUN_ITEMS
            ...
            _LOG_CARGO[index][item] = int(_LOG_CARGO[index].get(item, 0) or 0) + quantity
    _LOG_CARGO = _LOG_CARGO[:len(current_positions)]
    _LOG_PREV_OPPONENT = copy.deepcopy(opponent)
    _LOG_LAST_STEP = step


def _log_declared_sale(action, item):
    return any(
...
def _log_planned_quantity(item, start, stop):
    total = 0
    for future in range(start, min(stop, len(_TRACE))):
        for order in _TRACE[future].get("market", []) or []:
            if (
                isinstance(order, list) and len(order) >= 3
                and order[0] == "SELL" and order[1] == item
            ):
...
def _logistics_front_run(action, obs, opponent, step):
    orders = list(action.get("market", []) or [])
    if len(orders) >= 10:
        return
    sheds = _log_shed_tiles(len(opponent.get("tiles") or []))
    risks = []
    if _LOG_AUTO_SHED_STEP == step:
        for item, quantity in _LOG_AUTO_SHED.items():
            if item in _FRONT_RUN_ITEMS and quantity > 0:
                risks.append((float(quantity), item, int(quantity), _LOG_HORIZON))
    for index, cargo in enumerate(_LOG_CARGO[:len(_log_positions(opponent))]):
        if not cargo:
            continue
        x, y = map(int, _log_positions(opponent)[index])
        distance = min(abs(x - sx) + abs(y - sy) for sx, sy in sheds)
        if not 1 <= distance <= _LOG_HORIZON:
            continue
        for item, quantity in cargo.items():
            if item in _FRONT_RUN_ITEMS and quantity > 0:
    ...
    shed = (obs.get("private") or {}).get("shed") or {}
    choices = []
    for risk, item, _opponent_quantity, distance in risks:
        if _log_declared_sale(action, item):
            continue
        planned = _log_planned_quantity(item, step + 1, step + distance + 1)
...
def agent(obs, config=None):
    global _LAST_STEP, _CLONE_CONFIDENCE, _LOG_PREV_OPPONENT, _LOG_CARGO, _LOG_LAST_STEP
    global _LOG_AUTO_SHED, _LOG_AUTO_SHED_STEP
    step = min(int(obs.get("step", 0) or 0), len(_TRACE) - 1)
    if step == 0 or step <= _LAST_STEP:
        _CLONE_CONFIDENCE = 0
    if step == 0 or step <= _LOG_LAST_STEP:
        _LOG_PREV_OPPONENT = None
        _LOG_CARGO = []
        _LOG_LAST_STEP = -1
        _LOG_AUTO_SHED = {}
    ...
    _observe_opponent_cargo(opponent, step)
    ...
    _logistics_front_run(action, obs, opponent, step)
    return action
